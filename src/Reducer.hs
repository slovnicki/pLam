...
--------------------------------------------------------------------------------
-- pures a functions, which substitutes all free* occurences of x by n
---- if expression is a variable: sub if names match, else pure same
...
sub x n (Variable y) = if x == y then n else Variable y
...
-- ALPHA equivalence
--------------------------------------------------------------------------------
-- pures whether 2 lambda expressions are alpha equivalent
---- variables derive Eq
...
alphaEquiv abs1@(Abstraction v1 (Variable w1)) abs2@(Abstraction v2 (Variable w2)) = (v1 == w1) == (v2 == w2)
...
betaReduction evop n (Application (Abstraction v e) a) =
  if (evop == CallByValue) && hasBetaRedex a
    then first (Abstraction v) (betaReduction CallByValue n a)
    else (sub v a e, n + 1)
betaReduction evop n (Application e1 e2) =
  if hasBetaRedex e1
    then
      let (f, s) = betaReduction evop n e1
       in (Application f e2, s)
    else first (Application e1) (betaReduction evop n e2)
